...
function usage()
{
    echo "$cmdname <norm> <matrixdir> <hic file> <binsize> <build> <juicer> <chr> <outname>" 1>&2
}
...
do
    ...
    do
       ...
       $juicertool dump $type $norm $hic $chr $chr BP $binsize $tempfile -d &> info.$uniqID.temp
       if [ $? -ne 0 ]; then
       ...
       if test -s $tempfile; then
        ...
        rm rowname.$uniqID.temp pluscol.$uniqID.temp colname.$uniqID.temp $tempfile info.$uniqID.temp
    done
